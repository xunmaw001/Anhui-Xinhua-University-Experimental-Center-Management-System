...
<script>
    ...
    function setDataRow(item, number) {
        ...
        for (var i = 0; i < lxTypesOptions.length; i++) {
            if(lxTypesOptions[i].codeIndex == item.lxTypes){//下拉框value对比,如果一致就赋值汉字
                    lxTypesCell.innerHTML = lxTypesOptions[i].indexName;
            }
        }
    ...
    function remove(id) {
        ...
        if (mymessage == true) {
            ...
            httpJson("yiqi/delete", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "yiqi");
